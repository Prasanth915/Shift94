...
export const CreateProject = () => {
  ...
  useEffect(() => {
    if (repoMode === 'CREATE' && !repoNameManuallyEdited && projectTitle) {
  ...
  }, [projectTitle, repoMode, repoNameManuallyEdited]);
  ...
  const handleCreateRepository = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Pre-append to local options list immediately so Select can select it
        ...
        // Auto-select the newly created repository in authoritative form state
        setValue('githubUrl', newRepoUrl);
        setValue('sourceRepository', JSON.stringify(metadata));

        // Reset creation form & switch back to select mode
        setRepoMode('SELECT');
        setNewRepoName('');
        setNewRepoDescription('');
        setRepoNameManuallyEdited(false);

        // Fetch refreshed repositories list in the background
  ...
  const onSubmit = async (data) => {
    ...
    // Prevent project submission if GitHub publishing is selected but githubUrl is empty
    if (selectedPlatforms.includes('GITHUB') && !data.githubUrl) {
      toast.error('Please link a GitHub repository before publishing to GitHub.');
      return;
    }
